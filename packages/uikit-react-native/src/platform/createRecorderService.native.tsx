...
import type * as AudioRecorderPlayerModule from 'react-native-audio-recorder-player';
import type * as NitroSoundModule from 'react-native-nitro-sound';
...
export type AudioRecorderModule = typeof AudioRecorderPlayerModule | typeof NitroSoundModule;
...
class AudioRecorderPlayerAdapter implements RecorderModuleAdapter {
  private module: InstanceType<typeof AudioRecorderPlayerModule.default>;
  ...
  async startRecorder(uri: string): Promise<void> {
    const recordPath = Platform.OS === 'ios' ? uri.split('/').pop() || uri : uri;
    await this.module.startRecorder(recordPath, this.audioOptions as Parameters<typeof this.module.startRecorder>[1]);
  }
...
class NitroSoundAdapter implements RecorderModuleAdapter {
  private module: ReturnType<typeof NitroSoundModule.createSound>;
  private readonly audioOptions;
...
const createNativeRecorderService = (modules: Modules): RecorderServiceInterface => {
  const adapter = isNitroSoundModule(modules.audioRecorderModule)
    ? new NitroSoundAdapter(modules.audioRecorderModule)
    : new AudioRecorderPlayerAdapter(modules.audioRecorderModule as typeof AudioRecorderPlayerModule);

  return new VoiceRecorder(adapter, modules.permissionModule);
};

function isNitroSoundModule(module: AudioRecorderModule): module is typeof NitroSoundModule {
  return 'createSound' in module && typeof module.createSound === 'function';
}
